    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if isinstance(insect, QueenAnt):
                if insect.first_Queen and not QueenAnt.first_Queen:
                    return
...
class Bee(Insect):
    ...
    def __init__(self, armor, place=None, been_scared=False, forward=True):
        """Create an Insect with an ARMOR amount and a starting PLACE."""
        self.armor = armor
        self.place = place  # set by Place.add_insect and Place.remove_insect
        self.been_scared = been_scared
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        counter = 0
        position = self.place
        while position and position != beehive:
            if len(position.bees)>0 and counter>=self.min_range and counter <=self.max_range:
...
# The WallAnt class
class WallAnt(Ant):
    """WallAnt has a high armor value and does nothing each turn"""
...
class BodyguardAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 9
        "*** YOUR CODE HERE ***"
        if not self.contained_ant and not other.is_container: # maybe change
            return True
        return False
        # END Problem 9
...
class ScubaThrower(ThrowerAnt):
    """Implements a watersafe version of ThrowerAnt"""
...
# END Problem 13
    ...
    def __init__(self, armor=1, first_Queen=True):
        # BEGIN Problem 13
        "*** YOUR CODE HERE ***"
        self.armor = armor
            ...
            if current_place:
                if current_place.ant is not None:
                    if isinstance(current_place.ant, BodyguardAnt):
                        if current_place.ant.contained_ant and not isinstance(current_place.ant.contained_ant, FireAnt):
    ...
    def reduce_armor(self, amount):
        ...
        Insect.reduce_armor(self, amount)
        if self.armor <= 0:
            if self.first_Queen and not QueenAnt.first_Queen:
                bees_win()
        # END Problem 13
...
def make_scare(action, bee, duration):
    ...
    "*** YOUR CODE HERE ***"
    def scare(colony):
        if bee.place.exit is not colony.beehive:
            bee.forward = False
            action(colony)
            bee.forward = True
...
def apply_effect(effect, bee, duration):
    ...
    new_action = effect(original_action, bee, duration)
